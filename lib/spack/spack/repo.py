...
class FastPackageChecker(collections.abc.Mapping):
    ...
    def _create_new_cache(self) -> Dict[str, os.stat_result]:
        ...
        for file_name in os.listdir(self.packages_path):
            ...
            if not nm.valid_module_name(file_name, self.package_api):
                if not file_name.startswith(".") and file_name != "repo.yaml":
                    ...
                    tty.warn(
                        ...
                        f"package module name for Package API v{x}.{y}."
                    )
                continue
            ...
            cache[nm.mod_to_pkg_name(file_name, self.package_api)] = sinfo
